...
const mockQualities: QualityEntry[] = [
  {
    week: 1,
    year: 2024,
    message: "El arte de comenzar cada día con la curiosidad de un niño y la sabiduría de quien ha vivido."
  },
  {
    week: 5,
    year: 2024,
    message: "La paciencia no es la capacidad de esperar, sino la capacidad de mantener una buena actitud mientras se espera."
  },
  {
    week: 12,
    year: 2024,
    message: "Los sueños no tienen fecha de vencimiento. Solo requieren el coraje de perseguirlos cada día."
  },
  {
    week: 18,
    year: 2024,
    message: "La verdadera fortaleza no se mide por lo que puedes cargar, sino por lo que puedes dejar ir."
  },
  {
    week: 25,
    year: 2024,
    message: "Cada conversación es una oportunidad de construir puentes o de crear distancias. Elige sabiamente."
  },
  {
    week: 30,
    year: 2024,
    message: "La creatividad florece en los espacios donde el juicio se suspende y la curiosidad toma el control."
  },
  {
    week: 2,
    year: 2025,
    message: "El año nuevo no es solo un cambio de calendario, sino una invitación a reescribir tu historia."
  },
  {
    week: 8,
    year: 2025,
    message: "La gratitud transforma lo que tenemos en suficiente, y lo suficiente en abundancia."
  }
];
...
export async function GET() {
    ...
    if (!process.env.NOTION_API_KEY || !process.env.NOTION_DB_ID || 
        process.env.NOTION_API_KEY === 'dummy_key' || process.env.NOTION_DB_ID === 'dummy_db_id') {
